    ...
    <div class="setup-shell">
        <v-card class="setup-card pa-6" outlined>
            ...
            <v-form @submit.prevent="createAccount">
                <v-text-field v-model="form.email" label="Registered Email" type="email" outlined disabled dense dark readonly required />

                <v-text-field v-model="form.password" label="Password" :type="showPassword ? 'text' : 'password'" outlined dense dark required :append-icon="showPassword ? 'mdi-eye-off' : 'mdi-eye'" @click:append="showPassword = !showPassword" />

                <v-text-field v-model="form.confirmPassword" label="Confirm Password" :type="showConfirmPassword ? 'text' : 'password'" outlined dense dark required :append-icon="showConfirmPassword ? 'mdi-eye-off' : 'mdi-eye'" @click:append="showConfirmPassword = !showConfirmPassword" />

                <v-btn block large color="#C6FF00" class="black--text font-weight-bold mt-2" type="submit" :loading="loading">
                    Create Account
                </v-btn>
            </v-form>
...
export default {
    ...
    methods: {
        async onFirebaseAuthSuccess(firebaseUser) {
            try {
                const {
                    data
                ...
                console.log("UID linked:", data.user);
                if (data.message === "Firebase UID updated successfully") {
                ...
                if (error.response && error.response.status === 404) {
                    ...
                    console.error("Failed to link UID:", error);
                }
            }
        },
        async createAccount() {
            ...
            try {
                ...
                this.onFirebaseAuthSuccess(userCredential.user);
                setTimeout(() => {

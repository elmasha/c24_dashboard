            ...
            <v-card class="panel-card pa-4" outlined>
                <div class="panel-head mb-3 search-head">
                    ...
                    <div class="search-box-wrap">
                        <v-text-field v-model="searchQuery" dense outlined hide-details clearable prepend-inner-icon="mdi-magnify" label="Search UID, name, location, category" class="machine-search" @keyup.enter="searchMachines" @change="searchMachines" @click:clear="clearSearch" />
                ...
                <v-simple-table class="table-dark">
                    ...
                    <tbody>
                        <tr v-for="machine in machines" :key="machine.id">
                            ...
                            <td>
                                <span class="status-pill" :class="machine.status === 'online' ? 'status-online' : 'status-offline'">
                                    {{ machine.status }}
                                </span>
                            </td>
                            <td>
                                <span class="mode-pill" :class="machine.status_mode === 'manual' ? 'mode-manual' : 'mode-auto'">
                            ...
                            <td class="text-right">
                                <div class="action-wrap">
                                    ...
                                    <v-btn x-small text color="#7CFF8A" class="mr-1 mb-1" @click="setMachineStatus(machine, 'online')" :disabled="loadingId === machine.id">
                                        Set Online
                                    </v-btn>

                                    <v-btn x-small text color="#FF7B7B" class="mr-1 mb-1" @click="setMachineStatus(machine, 'offline')" :disabled="loadingId === machine.id">
                                        Set Offline
                                    </v-btn>

                                    <v-btn x-small text color="#FFD54F" class="mr-1 mb-1" @click="setMachineAuto(machine)" :disabled="loadingId === machine.id">
                                        Auto
                                    </v-btn>

                                    <v-btn x-small text color="#ff6b6b" class="mb-1" @click="deleteMachine(machine)" :disabled="loadingId === machine.id">
                                        Delete
                                    </v-btn>
                                </div>
                            </td>
    ...
    methods: {
        ...
        async searchMachines() {
            try {
                ...
                const {
                    data
                } = await api.get("/api/machines/search", {
                    params: {
